// src/workers/reprojectionWorker.js
...
self.onmessage = async (e) => {
  const { imageBitmap, extent, width, height } = e.data;
  ...
  // 使用 OffscreenCanvas 绘制原图
  const srcCanvas = new OffscreenCanvas(w_4326, h_4326);
  const srcCtx = srcCanvas.getContext("2d");
  srcCtx.drawImage(imageBitmap, 0, 0);
  const srcData = srcCtx.getImageData(0, 0, w_4326, h_4326);

  const destCanvas = new OffscreenCanvas(w_3857, h_3857);
