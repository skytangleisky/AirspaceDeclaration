<template>
    <div class="dragDialog absolute wstd-container" style="width:fit-content;height: fit-content;pointer-events: none;">
        ...
        <div class="bottom wstd-content" style="position: relative;pointer-events: auto;" v-if="tabActive">
            ...
            <div class="bottom-content">
                ...
                <div class="contain" @mousedown.stop>
                    <el-empty v-if="options.list.length ==0" :image-size="64"/>
                    <div
                        v-else
                        @scroll="scrolling"
                        class=""
        ...
        <Frame title="作业点筛选" v-model:render="show" width="400px" height="500px">
            <ZydFilter ></ZydFilter>
        </Frame>
    </div>
</template>
<script lang="ts" setup>
    ...
    watch([() => props.menus, () => options.value], ([result, value]) => {
        options.list = result.filter((item) => item.strName.indexOf(value) > -1 || item.strID.indexOf(value) > -1 || item.strCode.indexOf(value) > -1);
    }, {
        immediate: true,
    });
