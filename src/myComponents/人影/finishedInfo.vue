<template>
    <div :class="{'outer-container':props.isSinglePage}">
        <div :class="containerClass">
            <el-form inline>
                <el-form-item label="时间" style="width: 4rem;">
                    <el-date-picker
                        v-model="range"
                        type="datetimerange"
                ...
                <el-form-item label="作业点" style="width: 2.5rem;">
                    <el-select
                        clearable
                        v-model="zydID"
                        filterable
          ...
          <span style="color:var(--el-text-color-secondary)">{{ item.count }}</span>
                ...
                <el-button style="width:.8rem;" type="primary" @click="handleClick">新增</el-button>
            </el-form>
        </div>
        <div :class="{'box-container':props.isSinglePage}" class="table-box">
            <el-table :data="tableData" style="white-space: nowrap;">
                ...
                <el-table-column prop="timeLen" label="作业时长（秒）" width="130"/>
            ...
            <el-pagination :size="props.isSinglePage?'default':'small'" v-model:current-page="pageOption.page" :page-size="pageOption.size"
...
<script lang="ts" setup>
    ...
    const containerClass = computed(() => props.isSinglePage ? 'box-container' : 'search-box')
    const addShow = ref(false)
    const confirmShow = ref(false)
    const viewShow = ref(false)
    const rowData = ref<any>({})
    const Tomorrow = moment().add(1, 'day')
    const range = ref<[Date, Date]>([new Date(moment().year(), moment().month(), moment().date(), 0, 0), new Date(Tomorrow.year(), Tomorrow.month(), Tomorrow.date(), 0, 0),])
    const now = new Date()
    const defaultDates = [new Date(now.getFullYear(), now.getMonth() - 1, 1), // 上个月
