      ...
      <div class="item-right-top">
        ...
        <div class="item-right-top-item">
          ...
          <template v-else>
            <div>批准时间</div>
            <div class="top-item-value">
              {{item.tmAnswerRev ? item.tmAnswerRev.substring(11,16): ""}}
            </div>
          </template>
        </div>
        <div class="item-right-top-item">
...
<script lang="ts" setup>
  ...
  const beginText = computed(()=>(item: any) =>{
    if (工作状态格式化(item.ubyStatus) == "作业批准") {
      let seconds = moment(item.tmBeginAnswer).diff(now.value, "seconds");
      if(seconds<=0){
        seconds = 0
        修改作业状态数据(91,item.strWorkID)
      }
