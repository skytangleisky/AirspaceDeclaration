...
service.interceptors.request.use(
  config => {
    ...
    config.headers['database'] = config.headers['database'] || JSON.stringify({"host":"192.168.0.240","port":3306,"user":"root","password":"mysql","database":"ryplat_scry","type":"mysql"})//四川测试
    // config.headers['database'] = config.headers['database'] || JSON.stringify({"host":"10.194.22.240","port":33060,"user":"root","password":"SCRY@ryb1234","database":"ryplat_scry","type":"mysql"})//初验
    return config
  },
  error => {
