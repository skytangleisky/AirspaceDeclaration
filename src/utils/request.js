...
service.interceptors.request.use(
  config => {
    ...
    // config.headers['database'] = config.headers['database']||JSON.stringify({"host":"192.168.0.240","port":3306,"user":"root","password":"mysql","database":"ryplat_shanxi","type":"mysql"})//陕西测试
    ...
    config.headers['database'] = config.headers['database']||JSON.stringify({"host":"192.168.0.240","port":3306,"user":"root","password":"mysql","database":"ryplat_gansu","type":"mysql"})//陕西测试
    // config.headers['database'] = config.headers['database']||JSON.stringify({"host":"127.0.0.1","port":3306,"user":"root","password":"mysql","database":"ryplat_gansu","type":"mysql"})//陕西测试
    return config
  },
  error => {
